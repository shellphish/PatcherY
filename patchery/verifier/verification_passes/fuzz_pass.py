...
class FuzzVerificationPass(BaseVerificationPass):
    ...
    def _prepare_seeds(
        self, make_dummy_input=False, save_loc: Path = None, zip_seeds=True
    ) -> Path:
        ...
        inputs = []
        if (
            hasattr(self._prog_info, "_crashing_inputs")
            and self._prog_info._crashing_inputs
        ):
        ...
        if zip_seeds:
            ...
            corpus_zip = (
                Path(corpus_dir)
                / f"{self._prog_info.poi_report.cp_harness_name}_seed_corpus.zip"
            )
            save_loc = corpus_zip
            ...
            raise ValueError(
                "Either zip_seeds must be True or save_loc must be provided."
            )
    ...
    def _setup_fuzzer(
        self,
        timeout=TOTAL_FUZZING_TIME,
        use_aijon: bool = USE_AIJON,
        sync_inst_dir: Path = None,
    ) -> tuple[InstrumentedOssFuzzProject, Path, dict]:
        ...
        if is_java:
            ...
            corpus_location = self._prepare_seeds(
                zip_seeds=False, save_loc=corp_save_loc
            )

            fuzz_envs["FUZZING_ENGINE"] = "libfuzzer"
            fuzz_envs["ARTIPHISHELL_JAZZER_BENIGN_SEEDS"] = str(corp_save_loc)
            fuzz_envs["ARTIPHISHELL_JAZZER_CRASHING_SEEDS"] = str(
                sync_inst_dir / "crashes"
            )
            if use_aijon:
                raise ValueError(
                    "AIJON instrumentation is not supported for Java projects."
                )
        elif is_binary:
            corpus_location = self._prepare_seeds(zip_seeds=True)
            fuzz_envs["FUZZING_ENGINE"] = (
                "shellphish_aflpp" if not use_aijon else "shellphish_aijon"
            )
        else:
        ...
        if use_aijon:
            ...
            patch_path = Path(
                annotate_from_patch(
                    patch_path,
                    self._prog_info.source_root,
                    self._prog_info.code._function_resolver,
            ...
            if not patch_data:
                _l.warning(
                    "AIJON instrumentation was requested, but the patch is empty."
                )
                raise RuntimeError("Empty patch after AIJON annotation.")

            if "IJON" not in patch_data:
                _l.warning(
                    "AIJON instrumentation was requested, but the patch does not contain AIJON annotations."
                )
                raise RuntimeError(
                    "AIJON annotations not found in the patch after AIJON annotation."
                )
        elif is_binary:
        ...
        build_res = instr_project.build_target(
            patch_path=patch_path, sanitizer=self._prog_info.poi_report.sanitizer
        )
    ...
    def _fuzz_core(
        ...
        fuzzer, seed_corpus_dir, fuzz_envs = self._setup_fuzzer(
            timeout=timeout, use_aijon=use_aijon, sync_inst_dir=sync_inst_dir
        )
        ...
        def _run_fuzzer():
            ...
            if self.SAVE_CRASHES:
                ...
                if crash_dir_path.exists() and crash_dir_path.is_dir():
                    ...
                    _l.error(
                        "Somehow the crash directory %s does not exist or is not a directory.",
                        crash_dir_path,
                    )
    ...
    def _wait_for_valid_crash(
        self, crash_dir_path, fuzzing_thread, timeout=TOTAL_FUZZING_TIME
                ...
                if crash_file.is_file() and crash_file.stat().st_size > 0:
                    ...
                    if crashes and stack_trace:
                        if self._crash_in_relevant_location(stack_trace):
                            _l.info(
                                f"Found a crash input that reproduces the issue: {crash_file}"
                            )
                            crash_found = True
                            break
    ...
    def _fuzz_for_crashes(self, timeout=TOTAL_FUZZING_TIME):
        ...
        _l.info(
            "Fuzzing patch %s with fuzzer %s for %d seconds",
            self._patch.diff,
            self._prog_info.poi_report.fuzzer,
            timeout,
        )
        with FUZZING_LOCK:
            if self._verifier._patcher and not self._verifier._patcher.should_work:
                _l.warning(
                    "The patcher is shutting down all threads! Stopping fuzzing..."
                )
            ...
            if use_aijon:
                try:
                    fuzz_thread = self._fuzz_core(
                        instance_name,
                        sync_dir,
                        timeout=timeout,
                        use_aijon=True,
                        threaded=self.THREAD_FUZZING,
                    )
                except Exception as e:
                    fuzz_failed = True
                    _l.critical(
                        f"Failed to fuzz patch {instance_name}: {e} with AIJON instrumentation. Falling back to AFL++..."
                    )
            ...
            valid_crash_path, crash_info = self._wait_for_valid_crash(
                crash_dir_path,
                fuzz_thread,
                timeout=(timeout if self.THREAD_FUZZING else 4),
            )
        ...
        if valid_crash_path is None:
            if len(list(crash_dir_path.iterdir())) != 0:
                _l.warning(
                    "Unable to reproduce a crash with any fuzzer found crash. The patch may be unstable, but no valid crash inputs were found."
                )
    ...
    def _crash_in_relevant_location(self, stack_trace: list[str]) -> bool:
        ...
        patched_functions = [
            f.function_name
            for f in self._patch.patched_functions
            if f and f.function_name
        ]
        ...
        if any(func in stack_trace_slice for func in patched_functions):
            _l.info(
                "Fuzzer discovered crash in a patched function: %s", stack_trace_slice
            )
            return True

        if (
            stack_trace_slice
            and self._prog_info.crashing_function == stack_trace_slice[0]
        ):
            _l.info(
                "Fuzzer discovered crash in the original crashing function: %s",
                self._prog_info.crashing_function,
            )
            return True
    ...
    def run_pov(self, pov_file: Path) -> tuple[bool, str, list[str]]:
        ...
        exit_type, pov_report, stack_trace_funcs = self._prog_info.generates_alerts(
            input_obj
        )
    ...
    def should_skip(self):
        if not self._prog_info.crashing_function:
            _l.critical(
                "No crashing function set in program info, cannot run fuzz verification pass."
            )
