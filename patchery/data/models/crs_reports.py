...
from patchery.data.models.constraints import (
    PDT_ID,
    MD5_CONSTRAINTS,
    SHA1_CONSTRAINTS,
)
from patchery.data.models.crash_reports import (
    CallTrace,
    DedupSanitizerReport,
    SanitizerReport,
)
from patchery.data.models.symbols import POI
from patchery.data.models.target import HarnessInfo, CrashingInputMetadata
from patchery.data.models.indexer import GlobalVariableReference
from patchery.data.models.organizer_evaluation import (
...
class POIReport(HarnessInfo):
        ...
        if kind in (DedupInfoKind.FULL, DedupInfoKind.SHELLPHISH, DedupInfoKind.FULL):
            for reason, ct in self.stack_traces.items():
                getter = {
                    ...
                    DedupInfoKind.SHELLPHISH: lambda: ct.get_dedup_token_shellphish(
                        num_entries=3
                    ),
            ...
            dedup_tokens["significance"] = str(
                self.organizer_crash_eval.significance.value
            )
            dedup_tokens["crash-state"] = self.organizer_crash_eval.crash_state
            if self.organizer_crash_eval.instrumentation_key:
                dedup_tokens["instrumentation-key"] = (
                    self.organizer_crash_eval.instrumentation_key
                )
    ...
    def get_all_dedup_infos(self) -> List[DedupInfo]:
        return [self.get_dedup_info(kind) for kind in DedupInfoKind]
...
class AIxCCDedupValues(ShellphishBaseModel):
    significance: SignificanceEnum = Field(
        description="The exit code of the deduplication values"
    )
    crash_state: List[str] = Field(
        description="The crash state of the deduplication values"
    )
    instrumentation_state: List[str] = Field(
        description="The instrumentation state of the deduplication values"
    )
...
class RawPoVReport(ShellphishBaseModel):
    ...
    def clean_invalid_utf8(self):
        ...
        if self.unparsed is not None and isinstance(self.unparsed, bytes):
            self.unparsed = self.unparsed.decode("utf-8", errors="ignore").encode(
                "utf-8"
            )
...
class RunImageResult(ShellphishBaseModel):
    ...
    run_exit_code: Optional[int] = Field(
        description="The exit code of the command run in the background", default=None
    )
...
class RunImageInBackgroundResult(ShellphishBaseModel):
    task_success: bool = Field(description="Whether the task was successful")
    run_exit_code: int = Field(
        description="The exit code of the command run in the background"
    )
    time_scheduled: float = Field(description="The time the command was scheduled")
    time_start: float = Field(description="The time the command started")
...
class PoVReport(CrashingInputMetadata, RawPoVReport):
    ...
    def get_dedup_info(self, kind: Union[DedupInfoKind, str]) -> DedupInfo:
        ...
        if kind in (
            DedupInfoKind.FULL,
            DedupInfoKind.SHELLPHISH,
            DedupInfoKind.OSS_FUZZ,
        ):
            stack_traces = {}
...
class LocationOfInterest(ShellphishBaseModel):
    ...
    signature: str = Field(
        description="The signature of the point of interest", default=""
    )
...
class KumushiCodeFunction(ShellphishBaseModel):
    ...
    global_vars: list[GlobalVariableReference] | None = Field(
        default=None, description="The global variables of the function"
    )
...
class KumushiPOI(ShellphishBaseModel):
    ...
    crash_line_number: int | None = Field(
        default=None, description="The line number of the crash"
    )
    crash_line: str | None = Field(
        default=None, description="The line text of the crash"
    )
    code_function: KumushiCodeFunction = Field(
        description="The code function of the POI"
    )
...
class KumushiPOICluster(ShellphishBaseModel):
    ...
    reasoning: str | None = Field(
        default=None, description="The reasoning behind the cluster"
    )


class KumushiRootCauseReport(ShellphishBaseModel):
    poi_clusters: List[KumushiPOICluster] = Field(
        description="The Kumushi POI clusters"
    )
    rca_hash: str = Field(description="The hash of the RCA")


class PatchRequestMeta(ShellphishBaseModel):
    request_type: Literal["refine", "patch"] = Field(
        description="The type for this request (refine/patch)"
    )
    poi_report_id: str = Field(description="The poi report ID")
    failed_functionality: bool = Field(
        description="Whether the patch failed", default=False
    )
    patcher_name: str | None = Field(
        description="The patcher that create the previous patch", default=None
    )
    patch_id: str | None = Field(
        description="The id of patch that need to be refined", default=None
    )
    bucket_id: str | None = Field(description="The bucket ID", default=None)


class RunTestsResult(ShellphishBaseModel):
    tests_exist: bool = Field(
        description="True when the tests exist. When false, all other data is invalid"
    )
    timedout: bool = Field(
        description="True when the tests never finished. Normally should be False.",
        default=False,
    )
...
class RunOssFuzzBuildCheckResult(ShellphishBaseModel):
    ...
    internal_error: bool = Field(
        description="True when an internal error occurred during the build check",
        default=False,
    )
    stdout: str = Field(description="Tests stdout", default="")
    stderr: str = Field(description="Tests stderr", default="")
