...
class LLVMSymbolizerEntry(ShellphishBaseModel):
    ...
    def get_locations(self) -> Tuple[BinaryLocation, List[SourceLocation]]:
        ...
        source_locs = [symbol.to_location() for symbol in self.Symbol]
        assert all(
            loc.was_inlined for loc in self.Symbol[:-1]
        ), f"Only the last symbol can be non-inlined: {self.Symbol}, {source_locs}"
        assert (
            not source_locs or not self.Symbol[-1].was_inlined
        ), f"The last symbol should never be inlined: {self.Symbol}, {source_locs}"
