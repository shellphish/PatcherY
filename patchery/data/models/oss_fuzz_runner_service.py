...
from patchery.data.models.oss_fuzz import LanguageEnum, SanitizerEnum
# from shellphish_crs_utils.pydatatask import PDClient


class Base64Bytes(BaseModel):
    model_config = ConfigDict(
        extra="forbid", json_encoders={bytes: lambda v: base64.b64encode(v).decode()}
    )
    content: bytes
...
class ProjectTaskRequest(ShellphishBaseModel):
    ...
    quota: dict[str, Union[str, str]] = Field(
    ...
    resource_limits: Optional[dict[str, Union[str, str]]] = Field(
        default_factory=lambda: {
            "cpu": "10",
            "mem": "40Gi",
        }
