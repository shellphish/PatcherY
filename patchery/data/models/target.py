...
from patchery.data.models.oss_fuzz import ArchitectureEnum, SanitizerEnum
...
VALID_SOURCE_FILE_SUFFIXES = (
    VALID_SOURCE_FILE_SUFFIXES_C + VALID_SOURCE_FILE_SUFFIXES_JVM
)
...
class BuildInfoMixin:
    sanitizer: SanitizerEnum = Field(
        description="The sanitizer used in this target configuration"
    )
    architecture: ArchitectureEnum = Field(
        description="The architecture for this target configuration"
    )
...
class HarnessInfoMixin:
    ...
    cp_harness_binary_path: Path = Field(
        description="The challenge project harness binary path"
    )
    entrypoint_function: Optional[str] = Field(
        description="The entrypoint function of the harness", default=None
    )
    source_entrypoint: Optional[Path] = Field(
        description="The source file which contains the entrypoint of the harness",
        default=None,
    )
...
class HarnessInfo(
    ShellphishBaseModel, ProjectInfoMixin, BuildInfoMixin, HarnessInfoMixin
):
    build_configuration_id: PDT_ID = Field(
        description="The pydatatask build configuration id"
    )
    project_harness_metadata_id: Optional[PDT_ID] = Field(
        default=None, description="The pydatatask project harness metadata id"
    )
