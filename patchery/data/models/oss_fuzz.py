...
from patchery.data.models.symbols import SourceLocation
...
class OSSFuzzProjectYAML(ShellphishBaseModel):
    ...
    main_repo: Optional[str] = (
        None  # TODO: bring Url back, but has to handle git@github.com:asdf urls
    )
    vendor_ccs: Optional[List[str]] = None  # EmailStr
    sanitizers: Optional[List[Union[SanitizerEnum, SanitizerWithConfig]]] = Field(
        default=["address", "undefined"],
        description="Sanitizers the project supports, can opt-in to memory sanitizer or opt out of either ubsan or asan.",
    )
    architectures: Optional[List[ArchitectureEnum]] = Field(
        default=["x86_64"],
        description="Architectures the project supports, can opt-in to i386 or aarch64.",
    )
    fuzzing_engines: Optional[List[FuzzingEngineEnum]] = Field(
        default=["libfuzzer", "afl", "honggfuzz", "centipede"],
        description="Fuzzing engines the project supports, can opt-in to afl or honggfuzz.",
    )
    ...
    selective_unpack: Optional[bool] = (
        False  # Required to avoid out-of-space when executing AFL on clusterfuzz bots
    )
...
class ShellphishMetadata(ShellphishBaseModel):
    ...
    known_sources: Dict[str, Any] = Field(
        default_factory=dict, help="A mapping of known target sources to their contents"
    )
    files_by_type: Dict[str, int] = Field(
        default_factory=dict,
        help="A mapping of known file types to the count of them in the source repo",
    )
...
class Harness(ShellphishBaseModel):
    ...
    def from_project(project_dir: Path, harness_name: str) -> "Harness":
        ...
        if options_path.exists():
            harness.options = HarnessOptions.model_validate(
                ConfigParser().parse(options_path)
            )
