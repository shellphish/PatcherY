...
from patchery.data.function_resolver import FunctionResolver
from patchery.data.models import JavaInfo
from patchery.data.models.base import ShellphishBaseModel
from patchery.data.models.symbols import BinaryLocation, SourceLocation
...
class CallTraceEntry(ShellphishBaseModel):
    depth: int
    type: BacktraceType = Field(
        description="The type of the backtrace entry. Describes what this entry refers to (as best we can tell)."
    )

    trace_line: Optional[str] = Field(
        description="Either the function signature or the error line (Not currently used)"
    )
        ...
        try:
            ...
            if self.source_location:
                ...
                source_location = SourceLocation(
                    ...
                    if self.binary_location.package
                    else None,
                )
            else:
                log.warning(
                    "No source or binary location found for call trace entry: %s", self
                )
                return self

            try:
                results = function_resolver.resolve_source_location(
                    source_location, num_top_matches=3, allow_build_generated=False
                )
            except Exception:
                log.error(
                    "Error resolving source location: %s",
                    source_location,
                    exc_info=True,
                )
            ...
            log.error(
                "Results found for source location: loc=%s: results=%s",
                source_location,
                results,
            )
            ...
            try:
                focus_repo_keys = [
                    x[0]
                    for x in results
                    if function_resolver.get(x[0]).focus_repo_relative_path
                ]
            except Exception:
            ...
            if len(focus_repo_keys) > 1:
                ...
                log.warning(
                    "Multiple focus repo paths found for function name: %s => %s",
                ...
                if artiphishell_should_fail_on_error():
                    raise ValueError(
                        "Multiple focus repo paths found for function name: %s => %s"
                        % (source_location.function_name, results)
                    )
            key = None
            if focus_repo_keys:
            ...
            if not key and len(results) >= 2:
                ...
                log.warning(
                    "Multiple results found for function name: %s => %s",
                    source_location.function_name,
                    results,
                )
                log.warning("Picking the best one, but this is sus")
            ...
            if not key and len(results):
                ...
                log.info(
                    f"Lookup result function index for {source_location=} {key=} {rankings=}"
                )
            ...
            if func_index is not None:
                ...
                line_no = (
                    ...
                    if (
                        source_location.line_number is not None
                        and (
                            func_index.start_line
                            <= source_location.line_number
                            <= func_index.end_line
                        )
                    )
                ...
                line_map = {
                    func_index.start_line + i: code_line
                    for i, code_line in enumerate(func_index.code.split("\n"))
                }
                source_location = SourceLocation(
                    ...
                    file_name=Path(func_index.target_container_path.name)
                    if func_index.target_container_path
                    else None,
                    function_name=func_index.funcname,
                    line_number=line_no,
                    line_text=line_map[line_no] if line_no in line_map else None,
                    symbol_offset=self.source_location.symbol_offset
                    if self.source_location
                    else (
                        self.binary_location.symbol_offset
                        if self.binary_location
                        else None
                    ),
                    symbol_size=self.source_location.symbol_size
                    if self.source_location
                    else (
                        self.binary_location.symbol_size
                        if self.binary_location
                        else None
                    ),
                    ...
                    java_info=(
                        self.source_location.java_info if self.source_location else None
                    )
                    or JavaInfo(
...
class CallTrace(ShellphishBaseModel):
    reason: Optional[str] = Field(
        description="The reason for the call trace, normally a description of the crash"
    )
    dedup_token: Optional[str] = Field(
        default=None, description="A token that can be used to deduplicate call traces"
    )
    call_locations: List[CallTraceEntry] = Field(
        description="The call locations of the crash", default_factory=list
    )
    ...
    def get_dedup_token_shellphish(self, num_entries=3):
        ...
        while len(dedup_vals) < num_entries and i < len(self.call_locations):
            ...
            if entry.source_location and (
                entry.source_location.focus_repo_relative_path
                or entry.source_location.function_index_signature
            ):
                dedup_vals.append(entry.source_location.function_name)
            i += 1
        return "--".join(dedup_vals)
...
class LoSanMetaData(ShellphishBaseModel):
    sanitizer_type: LosanSanitizerEnum = Field(
        description="The type of losan sanitizer crash that was triggered",
        required=True,
    )
    ...
    def clean_invalid_utf8(self):
        if self.found_string is not None and isinstance(self.found_string, bytes):
            self.found_string = self.found_string.decode(
                "utf-8", errors="ignore"
            ).encode("utf-8")
        if self.expected_string is not None and isinstance(self.expected_string, bytes):
            self.expected_string = self.expected_string.decode(
                "utf-8", errors="ignore"
            ).encode("utf-8")
...
class SanitizerReport(ShellphishBaseModel):
    ...
    def clean_invalid_utf8(self):
        if self.raw_report is not None and isinstance(self.raw_report, bytes):
            self.raw_report = self.raw_report.decode("utf-8", errors="ignore").encode(
                "utf-8"
            )
    ...
    def sanity_check_model(self) -> "SourceLocation":
        ...
        assert self.losan == (
            self.losan_metadata is not None
        ), "If losan is set, losan_metadata must be set"
        assert self.losan == (
            b"[LOSAN]" in self.raw_report
        ), "losan should be set if and only if the report contains [LOSAN]"
        if self.losan:
            ...
            assert (
                "[LOSAN]" in self.summary
            ), "If losan is set, we should have [LOSAN] in the summary"
    ...
    def enhance_with_function_resolver(self, function_resolver: FunctionResolver):
        if not function_resolver:
            ...
            return
        assert (
            function_resolver is not None
        ), "Function resolver is required to enhance sanitizer report"
        ...
        for stack_trace_name, stack_trace in self.stack_traces.items():
            for cte in stack_trace.call_locations:
                try:
                    ...
                    log.error(
                        "Error enhancing call trace entry: %s", cte, exc_info=True
                    )
                    log.error("Error: %s", e)
                    if artiphishell_should_fail_on_error():
                        raise
...
def clean_java_report_str(report: str):
    ...
    report = re.sub(
        r'FOUND:\s+"(.*?)"\s+and\s+EXPECTED:\s+"(.*?)"',
        "FOUND: <REDACTED> and EXPECTED: <REDACTED>",
        report,
    )
    report = re.sub(r"-Xmx\d+[mMgGkK]", "-Xmx<REDACTED>", report)
    report = re.sub(
        r'java.io.IOException: Cannot run program ".*',
        "java.io.IOException: Cannot run program",
        report,
    )
    report = re.sub(
        r"PS Scavenge: \d+ collections took",
        "PS Scavenge: <REDACTED> collections took",
        report,
    )
    report = re.sub(
        r"Attempted connection to: .*", "Attempted connection to: <REDACTED>", report
    )
    report = re.sub(
        r"Index \d+ out of bounds for length \d+",
        "Index <REDACTED> out of bounds for length <REDACTED>",
        report,
    )
...
def clean_report(report: bytes):
    ...
    report = re.sub(
        r"SHELLPHISH_FOUND_LOSAN: \"(.*?)\"",
        'SHELLPHISH_FOUND_LOSAN: "<REDACTED>"',
        report,
    )
    report = re.sub(
        r"SHELLPHISH_EXPECTED_LOSAN: \"(.*?)\"",
        'SHELLPHISH_EXPECTED_LOSAN: "<REDACTED>"',
        report,
    )
...
class DedupSanitizerReport(ShellphishBaseModel):
    ...
    def from_sanitizer_report(cls, report: SanitizerReport):
        ...
        dedup_tokens_full = {
            k: v.get_dedup_token_full() for k, v in report.stack_traces.items()
        }
        dedup_tokens_shellphish = {
            k: v.get_dedup_token_shellphish() for k, v in report.stack_traces.items()
        }
        stack_traces = {}
        for k, v in report.stack_traces.items():
            ...
            stack_traces[k] = CallTrace(
                reason=v.reason,
                dedup_token=v.dedup_token,
