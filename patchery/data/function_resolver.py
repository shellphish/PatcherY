...
from patchery.data.models.constraints import PDT_ID
from patchery.data.models.symbols import RelativePathKind, SourceLocation
from patchery.data.models.indexer import (
    FunctionIndex,
    FUNCTION_INDEX_KEY,
)
from patchery.data.models.coverage import (
    CoverageLine,
    LinesCoverage,
    FunctionCoverageMap,
    FileCoverageMap,
)
from patchery.data.models.target import VALID_SOURCE_FILE_SUFFIXES
from patchery.utils import artiphishell_should_fail_on_error
import yaml
...
def get_function_name_match(
    source_location: SourceLocation, function_index_entry: FunctionIndex
) -> Optional[FunctionIndexRanking]:
    ...
    if (
        source_location.function_name.startswith("OSS_FUZZ_")
        and source_location.function_name[len("OSS_FUZZ_") :]
        == function_index_entry.funcname
    ):
        # special case function prefixes of OSS_FUZZ_ as e.g., used in libpng
        return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
    if source_location.function_name.endswith(
        "::" + function_index_entry.funcname
    ) or source_location.function_name.endswith("." + function_index_entry.funcname):
        return FunctionIndexRanking(MatchKind.MAYBE, 0.8)
    if function_index_entry.funcname.endswith(
    ...
    if source_location.function_name in function_index_entry.funcname:
        char_before = (
            source_location.function_name.index(function_index_entry.funcname) - 1
        )
        char_after = char_before + len(function_index_entry.funcname)
        char_before = (
            function_index_entry.funcname[char_before] if char_before >= 0 else " "
        )
        char_after = (
            function_index_entry.funcname[char_after]
            if char_after < len(function_index_entry.funcname)
            else " "
        )
        if char_before in ["_", " ", ":", "."] and char_after in ["_", " ", ":", "."]:
            ...
            return FunctionIndexRanking(
                MatchKind.MAYBE,
                0.4 if char_before != "_" and char_after != "_" else 0.3,
            )
    elif function_index_entry.funcname in source_location.function_name:
        char_before = (
            function_index_entry.funcname.index(source_location.function_name) - 1
        )
        char_after = char_before + len(source_location.function_name)
        char_before = (
            source_location.function_name[char_before] if char_before >= 0 else " "
        )
        char_after = (
            source_location.function_name[char_after]
            if char_after < len(source_location.function_name)
            else " "
        )
        if char_before in ["_", " ", ":", "."] and char_after in ["_", " ", ":", "."]:
            return FunctionIndexRanking(
    ...
    if (
        function_index_entry.funcname in source_location.function_name
        or source_location.function_name in function_index_entry.funcname
    ):
...
def get_relative_filename_match(
    source_location: SourceLocation, function_index_entry: FunctionIndex
) -> Optional[FunctionIndexRanking]:
    if (
        source_location.focus_repo_relative_path
        and function_index_entry.focus_repo_relative_path
        and source_location.focus_repo_relative_path
        == function_index_entry.focus_repo_relative_path
    ):
        return FunctionIndexRanking(
            MatchKind.DEFINITELY, 1.0
        )  # if the focus repo relative path is the same, we have a perfect match
    if source_location.relative_path:
        if (
            source_location.relative_path.name == "sqlite3.c"
        ):  # fork the amalgamation, holy shirt
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)

        if (
            function_index_entry.focus_repo_relative_path
            and source_location.relative_path
            == function_index_entry.focus_repo_relative_path
        ):
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        if str(function_index_entry.target_container_path).endswith(
            str(source_location.relative_path)
        ):
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        ...
        if str(source_location.relative_path) in str(
            function_index_entry.target_container_path
        ):
            matching, min_match = count_matching_final_path_parts(
...
def get_full_file_path_match(
    ...
    if source_location.full_file_path:
        if function_index_entry.target_container_path == source_location.full_file_path:
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        if (
            source_location.full_file_path.name == "sqlite3.c"
        ):  # fork the amalgamation, holy shirt
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        matching, min_match = count_matching_final_path_parts(
            source_location.full_file_path, function_index_entry.target_container_path
        )
        return FunctionIndexRanking(MatchKind.MAYBE, 0.8 * matching / min_match)


def get_filename_match(
    source_location: SourceLocation, function_index_entry: FunctionIndex
) -> Optional[FunctionIndexRanking]:
    if source_location.file_name:
        if source_location.file_name == function_index_entry.filename:
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        if (
            source_location.file_name == "sqlite3.c"
        ):  # fork the amalgamation, holy shirt
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        if (
            function_index_entry.focus_repo_relative_path
            and source_location.file_name
            == function_index_entry.focus_repo_relative_path.name
        ):
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        if (
            source_location.file_name.name
            == function_index_entry.target_container_path.name
        ):
            return FunctionIndexRanking(MatchKind.DEFINITELY, 1.0)
        return FunctionIndexRanking(
            MatchKind.DEFINITELY_NOT_IN_WELL_BEHAVED_SETTINGS, 0.0
        )


def get_line_number_match(
    source_location: SourceLocation, function_index_entry: FunctionIndex
) -> Optional[FunctionIndexRanking]:
    if source_location.line_number is not None:
        ...
        if (
            function_index_entry.start_line - 3
        ...
        if (
            function_index_entry.start_line - 3
            <= source_location.line_number
            <= function_index_entry.end_line + 3
        ):
...
def get_java_info_match(
    ...
    if source_location.java_info.class_name:
        ...
        cur = 0
        if (
            source_location.java_info.class_name + ".java"
            == function_index_entry.target_container_path.name
        ):
            cur += 1
        if (
            source_location.java_info.class_name == function_index_entry.class_name
            or function_index_entry.class_name.endswith(
                source_location.java_info.class_name
            )
        ):
            cur += 1

        class_name_root = function_index_entry.target_container_path.name.split(
            ".java"
        )[0]
        if source_location.java_info.class_name.startswith(class_name_root):
            cur += (
                0.8
                if source_location.java_info.class_name.startswith(
                    class_name_root + "$"
                )
                else 0.5
            )
    ...
    return (
        FunctionIndexRanking(MatchKind.MAYBE, matching / total) if total > 0 else None
    )
...
def function_index_to_source_location(
    key: FUNCTION_INDEX_KEY, index_obj: FunctionIndex
) -> SourceLocation:
...
class FunctionResolver:
    ...
    @abstractmethod
    def get_code(
        self, key: FUNCTION_INDEX_KEY
    ) -> Tuple[Optional[Path], Path, int, str]:
        raise NotImplementedError
    ...
    ) -> Tuple[Dict[FUNCTION_INDEX_KEY, FUNCTION_INDEX_KEY], List[FUNCTION_INDEX_KEY]]:
        """
        For each index:
    ...
    def find_matching_index(
        ...
        """
        matches, missing = self.find_matching_indices(
            [index], scope, can_include_self, can_include_build_generated
        )
        if len(matches) == 1:
    ...
    def find_functions_with_annotation(
        self, annotation: str
    ) -> Iterator[FUNCTION_INDEX_KEY]:
        raise NotImplementedError(
            "This method should be implemented in the subclass. It is not supported in the base FunctionResolver class."
        )

    def get_many(
        self, keys: List[FUNCTION_INDEX_KEY]
    ) -> Dict[FUNCTION_INDEX_KEY, FunctionIndex]:
        return {key: self.get(key) for key in keys}

    def get_with_default(
        self, key: FUNCTION_INDEX_KEY, default=None
    ) -> Optional[FunctionIndex]:
        try:
            return self.get(key)
        except KeyError:
            return default
    ...
    def get_focus_repo_keys(
        self, focus_repo_container_path: Optional[Union[Path, str]]
        ...
        if not focus_repo_container_path:
            raise ValueError(
                "No focus repo container path provided and no default set."
            )
        return [
            key for key in self.keys() if key.startswith(str(focus_repo_container_path))
        ]

    def get_filtered_keys(
        self, key_only_jq_filter_expression: str
    ) -> List[FUNCTION_INDEX_KEY]:
        """
        ...
        if key_only_jq_filter_expression not in self.cached_jq_filter_expression_keys:
            ...
            self.cached_jq_filter_expression_keys[key_only_jq_filter_expression] = (
                filtered_keys
            )
        return self.cached_jq_filter_expression_keys[key_only_jq_filter_expression]

    def get_filtered(
        self, key_only_filter_expression: str, full_filter_expression: str
    ) -> List[FUNCTION_INDEX_KEY]:
        """
        ...
        """
        if (
            key_only_filter_expression,
            full_filter_expression,
        ) not in self.cached_jq_filter_expression_keys:
            filtered_keys = self.get_filtered_keys(key_only_filter_expression)
            full_jq_filter = jq.compile(full_filter_expression)

            vals_to_filter = [
                {"key": str(key), "value": prepare_for_json(self.get(key).model_dump())}
                for key in filtered_keys
            ]
            is_valid = full_jq_filter.input_values(vals_to_filter).all()
            filtered_vals = {
                k["key"]: k["value"]
                for k, valid in zip(vals_to_filter, is_valid)
                if valid
            }

            self.cached_jq_filter_expression_keys[
                (key_only_filter_expression, full_filter_expression)
            ] = filtered_vals
        return self.cached_jq_filter_expression_keys[
            (key_only_filter_expression, full_filter_expression)
        ]
    ...
    def get_function_coverage_for_file(
        self,
        path: Union[Path, str],
        lines: LinesCoverage,
        function_keys_of_interest=None,
    ) -> FunctionCoverageMap:
        if path not in self.cached_lines_to_function:
            ...
            for key in self.find_by_filename(path):
                ...
                self.cached_lines_to_function[path].update(
                    {line: key for line in range(start, end + 1)}
                )
        ...
        for line in lines:
            if line.line_number not in self.cached_lines_to_function[path]:
                continue
            containing_function_key = self.cached_lines_to_function[path][
                line.line_number
            ]
            if (
                function_keys_of_interest is not None
                and containing_function_key not in function_keys_of_interest
            ):
                continue
            new = CoverageLine(
                line_number=line.line_number,
                count_covered=line.count_covered,
                code=self.get_function_code_line(
                    containing_function_key, line.line_number
                ),
            )
    ...
    def get_function_coverage(
        self,
        file_coverage: FileCoverageMap,
        path_suffixes_of_interest=None,
        function_keys_of_interest=None,
    ) -> FunctionCoverageMap:
        res = {}
        if function_keys_of_interest is not None:
            assert (
                path_suffixes_of_interest is None
            ), "Cannot specify both function keys and path suffixes of interest"
            path_suffixes_of_interest = [
                os.path.basename(self.get_target_container_path(key))
                for key in function_keys_of_interest
            ]
        for path, lines in file_coverage.items():
            if path_suffixes_of_interest is not None and not any(
                str(path).endswith(suffix) for suffix in path_suffixes_of_interest
            ):
                continue
            res.update(
                self.get_function_coverage_for_file(
                    path, lines, function_keys_of_interest=function_keys_of_interest
                )
            )
        return res

    def get_function_coverage_report(
        self,
        inputs,
        function_coverage: FunctionCoverageMap,
        keys_of_interest: List[FUNCTION_INDEX_KEY] = None,
    ):
        reports = []
        if not keys_of_interest:
            keys_of_interest = list(function_coverage.keys())
        for key in keys_of_interest:
            focus_repo_rel_path, target_container_path, func_start_line, func_code = (
                self.get_code(key)
            )
            ...
            if key not in function_coverage:
                report += "No coverage was reached in this function.\n"
                continue

            func_cov_lines = list(
                sorted(function_coverage[key], key=lambda x: x.line_number)
            )
            ...
            for i, line in enumerate(func_code.split("\n")):
                ...
                if (
                    func_cov_lines
                    and func_cov_lines[0].line_number == i + func_start_line
                ):
        ...
        report += "\n\n".join(
            ...
            if reports
                ...
                "ERROR: No coverage was reached in any of the requested functions. You should probably check the coverage of the harness or earlier functions to see where you are getting stuck."
            ]
        )
    ...
    def resolve_source_location(
        ...
        allow_build_generated: bool = False,
        focus_repo_only: bool = False,
        ...
        if srcloc.function_name:
            ...
            for key in self.find_by_funcname(srcloc.function_name):
                ...
                if focus_repo_only and self.get(key).focus_repo_relative_path is None:
                    continue
                if (
                    index_entry.class_name
                    and srcloc.java_info
                    and srcloc.java_info.class_path
                ):  # java info should be high-confidence, this cannot be a match
            ...
            contenders_to_rank = (
                perfect_function_matches
                if perfect_function_matches
                else imperfect_function_matches
            )
    ...
    def _rank_contenders(
        ...
        for key, ranking in contenders_to_rank:
            ...
            if relative_path_ranking := get_relative_filename_match(
                srcloc, self.get(key)
            ):
            ...
            if full_file_path_ranking := get_full_file_path_match(
                srcloc, self.get(key)
            ):
        ...
        rankings = sorted(
            rankings, key=lambda x: x[1], reverse=True
        )  # sort by highest ranking first
        ...
        log.debug(
            f"Returning the highest {num_top_matches} rankings: {rankings[:num_top_matches]}"
        )
        return [
            (r[0], [v[1] for v in r[2]]) for r in rankings[:num_top_matches]
        ]  # return the key and the rank values
...
class LocalFunctionResolver(FunctionResolver):
    def __init__(self, functions_index_path: str, functions_jsons_path: str):
        ...
        try:
            ...
            with open(self.functions_index_path, "r") as infile:
                self.functions_index: Dict[str, Path] = {
                    k: Path(v) for k, v in json.load(infile).items()
                }
        except Exception:
            log.warning("[INFO] 🔄 Not a full index, trying to load as a commit index.")
            log.warning("[INFO] 🔄 Attempting loading a commit index...")
            ...
            with open(self.functions_index_path, "r") as infile:
                ...
                try:
                    ...
                    thedata = {
                        key: value
                        for commit, commit_funcs_dict in thedata.items()
                        for key, value in commit_funcs_dict.items()
                    }
                    self.functions_index: Dict[str, Path] = {
                        k: Path(v) for k, v in thedata.items()
                    }
                except Exception as e:
                    log.critical(
                        "[CRITICAL] 🤯 Could not load a function index nor a commit index. Exiting."
                    )
                    log.critical(e)
                    import traceback
                    ...
                    traceback.print_exc()
                    raise ValueError(
                        "Could not load a function index nor a commit index. Exiting."
                    )
    ...
    def _get(self, key: FUNCTION_INDEX_KEY) -> FunctionIndex:
        ...
        if not (self.functions_jsons_path / self.functions_index[key]).exists():
            raise ValueError(
                f"Function jsons entry {self.functions_index[key]} does not exist for {key} at {self.functions_jsons_path}: {os.listdir(self.functions_jsons_path)}"
            )
    ...
    def __full_scan_of_doom_and_destruction__load_all_hashes_if_needed(
        self, scope: Literal["all", "focus", "non-focus", "compiled"] = "all"
    ):
    ...
    def get_code(self, key: FUNCTION_INDEX_KEY) -> Tuple[Path, Path, int, str]:
        if key not in self.cached_func_codes:
            idx = self.get(key)
            self.cached_func_codes[key] = (
                idx.focus_repo_relative_path,
                idx.target_container_path,
                idx.start_line,
                idx.code,
            )
    ...
    def find_by_funcname(self, s: str) -> Iterator[FUNCTION_INDEX_KEY]:
        if s not in self.cached_by_funcname:
            self.cached_by_funcname[s] = [
                key
                for key in self._find_matching_indices(s)
                if self.get_funcname(key).split("::")[-1] == s
            ]
    ...
    def find_functions_with_annotation(
        ...
        if annotation not in self.cached_with_annotation.keys():
            ...
            for key in self.functions_index.keys():
                func = self.get(key)
                if (
                    func.language_specific_info
                    and "annotations" in func.language_specific_info.keys()
                ):
    ...
    def find_matching_indices(
        ...
        if scope == "all":
            assert (
                not can_include_self
            ), "can_include_self=true on `all` scope will always return self..."
        ...
        to_find = [k for k in indices if k not in out_map and k not in missing]
        ...
        for goal_key in to_find:
            ...
            matches = {
                key
                for key in matches
                if is_in_scope(key) and allow_build_generated(key)
            }
            ...
            ), f"Could not find a best match for {goal_key}"
    ...
    def find_by_filename(self, s: Union[Path, str]) -> Iterator[FUNCTION_INDEX_KEY]:
        ...
        if basename not in self.cached_by_filename:
            self.cached_by_filename[basename] = list(
                self._find_matching_indices(basename)
            )
    ...
    def resolve_with_leniency(self, name: str) -> Iterator[FUNCTION_INDEX_KEY]:
        ...
        if match := re.fullmatch(r"source:(.+):(\d+):(\d+)::", name):
            ...
            filtered = [
                ...
                if self.get_function_boundary(k)[0]
                <= line_match
                <= self.get_function_boundary(k)[1]
            ]
            if len(filtered) > 0:
                yield from filtered
                return
            ...
            raise ValueError(
                f"This looks like a function index key but we could not find any function matching {name}. This function key does not exist, please move on."
            )
        ...
        if "::" in name:
            ...
            filtered = [
                k
                for k in possible_keys
                if self.get_funcname(k) == name
                and all(sub in k for sub in class_name.split("::"))
            ]
            if len(filtered) > 0:
                yield from filtered
                return
...
class RemoteFunctionResolver(FunctionResolver):
    def __init__(self, cp_name: str, project_id: Union[str, PDT_ID]):
        ...
        if os.getenv("CRS_TASK_NUM"):
            ...
            if "TASKNUM" in self.url:
                raise ValueError(
                    "Env CRS_TASK_NUM is not set but FUNC_RESOLVER_URL contains TASKNUM"
                )
    ...
    def is_ready(self) -> bool:
        ...
        result = r.json()
        if (
            result.get("status", None) == "error"
            and result.get("data", None) == "Server not initialized"
        ):
            return False
    ...
    def _make_request(self, endpoint: str, data: dict) -> dict:
        while True:
            ...
            if (
                ...
                log.warning(
                    f"Function resolver server not initialized, waiting 30 seconds before retrying /{endpoint}"
                )
                time.sleep(30)
                continue
            return result
    ...
    def keys(self):
        data = {"cp_name": self.cp_name, "project_id": self.project_id}
        ...
        if api_status == "error":
            ...
            assert (
                api_status == "success"
            ), f"API status code is not success|error: {result}"
            # This means the function was found in the index.
            # The response should be a dict with the function index
            return result.get("data", [])

    def _get(self, key: FUNCTION_INDEX_KEY) -> FunctionIndex:
        data = {"cp_name": self.cp_name, "project_id": self.project_id, "key": key}
        ...
        if api_status == "error":
            ...
            assert (
                api_status == "success"
            ), f"API status code is not success|error for {key}: {result}"
            # This means the function was found in the index.
            # The response should be a dict with the function index
    ...
    def get_many(self, keys):
        ...
        data = {"cp_name": self.cp_name, "project_id": self.project_id, "keys": keys}
        ...
        if api_status == "error":
            ...
            assert (
                api_status == "success"
            ), f"API status code is not success|error for {keys}: {result}"
            # This means the function was found in the index.
            # The response should be a dict with the function index
            return {
                key: FunctionIndex.model_validate(value)
    ...
    def get_focus_repo_keys(self, focus_repo_container_path):
        ...
        api_status = result.get("status", None)
        assert (
            api_status is not None
        ), f"API status code is None for {focus_repo_container_path}: {result}"
        if api_status == "error":
            raise KeyError(f"Function keys not found in index: {result}")
        else:
            assert (
                api_status == "success"
            ), f"API status code is not success|error for {focus_repo_container_path}: {result}"
            # This means the function was found in the index.
            return result.get("data", [])
    ...
    def get_filtered_keys(
        ...
        assert (
            api_status is not None
        ), f"API status code is None for {key_only_jq_filter_expression}: {result}"
        if api_status == "error":
            raise KeyError(f"Function keys not found in index: {result}")
        else:
            assert (
                api_status == "success"
            ), f"API status code is not success|error for {key_only_jq_filter_expression}: {result}"
            # This means the function was found in the index.
            return result.get("data", [])
    ...
    def get_filtered(
        ...
        assert (
            api_status is not None
        ), f"API status code is None for {key_only_filter_expression}: {result}"
        if api_status == "error":
            raise KeyError(f"Function keys not found in index: {result}")
        else:
            assert (
                api_status == "success"
            ), f"API status code is not success|error for {key_only_filter_expression}: {result}"
            # This means the function was found in the index.
            return {
                k: FunctionIndex.model_validate(v)
    ...
    def get_funcname(self, key: FUNCTION_INDEX_KEY) -> str:
        if key in self.cached_func_names:
            ...
            assert (
                api_status is not None
            ), f"API status code is None for {key}: {result}"
            ...
            if api_status == "error":
                ...
                ), f"Function name is None for {key}: {result}"
    ...
    def get_focus_repo_relative_path(self, key: FUNCTION_INDEX_KEY) -> Optional[Path]:
        if key in self.cached_focus_repo_relative_paths:
            ...
            if api_status == "error":
                ...
                self.cached_focus_repo_relative_paths[key] = (
                    RelativePathKind(rel_path[0]),
                    Path(rel_path[1]),
                )
    ...
    def get_target_container_path(self, key: FUNCTION_INDEX_KEY) -> Path:
        ...
        if api_status == "error":
            raise KeyError(
                f"Target container path for {key} not found in index: {result}"
            )
        else:
            assert (
                api_status == "success"
            ...
            assert (
                target_container_path is not None
            ), f"Target container path is None for {key}: {r.text} {result}"
    ...
    def get_code(self, key: FUNCTION_INDEX_KEY) -> Tuple[Path, Path, int, str]:
        if key in self.cached_func_codes:
            ...
            if api_status == "error":
                ...
                assert (
                    len(code) == 4
                ), f"Code is not a list of 4 elements for {key}: {result}"
                # Convert to a tuple RelativePathKind and Path objects
                self.cached_func_codes[key] = (
                    Path(code[0]) if code[0] else None,
                    Path(code[1]) if code[1] else None,
                    code[2],
                    code[3],
                )
                return self.cached_func_codes[key]
    ...
    def get_function_boundary(self, key: FUNCTION_INDEX_KEY) -> Tuple[int, int]:
        if key in self.cached_function_boundaries:
            ...
            if api_status == "error":
                ...
                # Users will have to handle this Exception themselves
                raise KeyError(
                    f"Function boundary for {key} not found in index: {result}"
                )
            else:
                assert (
                    api_status == "success"
    ...
    def find_functions_with_annotation(
        ...
        if annotation not in self.cached_with_annotation:
            ...
            assert (
                api_status is not None
            ), f"API status code is None for {annotation}: {result}"
            if api_status == "error":
                # This means the function was not found in the index.
                # Users will have to handle this Exception themselves
                raise KeyError(
                    f"No results for name {annotation} in function index: {result}"
                )
            else:
                assert (
                    api_status == "success"
                ), f"API status code is not success|error for {annotation}: {result}"
                # This means the function was found in the index.
                # The response should be a list of function keys
                matches = result.get("data", None)
                assert (
                    matches is not None
                ), f"matches is None for {annotation}: {result}"
    ...
    def find_by_funcname(self, s: str) -> Iterator[FUNCTION_INDEX_KEY]:
        ...
        if s not in self.cached_by_funcname:
            data = {"cp_name": self.cp_name, "project_id": self.project_id, "s": s}
            ...
            if api_status == "error":
                ...
                assert (
                    api_status == "success"
                ), f"API status code is not success|error for {s}: {result}"
                # This means the function was found in the index.
                # The response should be a list of function keys
                matches = result.get("data", None)
    ...
    def find_matching_indices(
        ...
        assert (
            api_status is not None
        ), f"API status code is None for {len(indices)} strings: {result}"
        if api_status == "error":
            raise Exception(
                f"Error in find_matching_indices for {len(indices)} strings: {result}"
            )

        assert (
            api_status == "success"
        ), f"API status code is not success|error for {len(indices)} strings: {result}"
    ...
    def find_by_filename(self, s: Union[Path, str]) -> Iterator[FUNCTION_INDEX_KEY]:
        ...
        if s not in self.cached_by_filename:
            data = {"cp_name": self.cp_name, "project_id": self.project_id, "s": str(s)}
            ...
            if api_status == "error":
                ...
                # Users will have to handle this Exception themselves
                raise KeyError(
                    f"No results for filename {s} in function index: {result}"
                )
            else:
                assert (
                    api_status == "success"
    ...
    def resolve_with_leniency(self, name: str) -> Iterator[FUNCTION_INDEX_KEY]:
        ...
        if name not in self.cached_leniency_resolutions:
            ...
            assert (
                api_status is not None
            ), f"API status code is None for {name}: {result}"
            if api_status == "error":
                # This means the function was not found in the index.
                # Users will have to handle this Exception themselves
                raise KeyError(
                    f"No results for name {name} in function index: {result}"
                )
            else:
                assert (
                    api_status == "success"
                ), f"API status code is not success|error for {name}: {result}"
                # This means the function was found in the index.
                # The response should be a list of function keys
                matches = result.get("data", None)
    ...
    def upload(
        self, full_functions_index_path: Path, full_functions_index_jsons_dir: Path
    ):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run(
                [
                    "tar",
                    "-cvf",
                    os.path.join(temp_dir, "functions_jsons/functions_jsons.tar"),
                    ".",
                ],
                check=True,
            ...
            subprocess.run(
                [
                    ...
                    "-cvf",
                    "data.tar",
                    "functions_index/functions_index.tar",
                    "functions_jsons/functions_jsons.tar",
                ],
                check=True,
                cwd=temp_dir,
            )
            ...
            with open(temp_dir / "data.tar", "rb") as f:
                response = requests.post(
                    f"{self.url}/init_server", data=data, files={"data": f}
                )
...
def function_resolver_upload():
    ...
    parser = argparse.ArgumentParser(
        description="Upload function index to the function resolver server"
    )
    parser.add_argument(
        "project_name", type=str, help="Name of the project (aka cp_name)"
    )
    parser.add_argument("project_id", type=str, help="Project ID")
    parser.add_argument(
        "full_functions_index_path", type=Path, help="Path to the full functions index"
    )
    parser.add_argument(
        "full_functions_index_jsons_dir",
        type=Path,
        help="Path to the full functions index jsons dir",
    )
    args = parser.parse_args()

    resolver = RemoteFunctionResolver(args.project_name, args.project_id)
    result = resolver.upload(
        args.full_functions_index_path, args.full_functions_index_jsons_dir
    )
    print(result)
...
def function_resolver_upload_backup():
    ...
    project_ids = []
    for f in os.listdir(
        args.backup_dir / "generate_full_function_index.target_functions_index"
    ):
        assert (
            f.split(".")[0] == f
        ), f"the type of target_functions_index has changed: {f}"
    ...
    for project_id in project_ids:
        with tempfile.TemporaryDirectory() as tempdir:
            if not os.path.isdir(
                args.backup_dir
                / "generate_full_function_index.target_functions_jsons_dir"
                / project_id
            ):
                tar_path = (
                    args.backup_dir
            ...
            full_functions_index_path = (
                args.backup_dir
                / "generate_full_function_index.target_functions_index"
                / project_id
            )
            crs_task = (
                args.backup_dir
                / "generate_full_function_index.crs_task"
                / f"{project_id}.yaml"
            )
            ...
            result = resolver.upload(
                full_functions_index_path, full_functions_index_jsons_dir
            )
            print(result)
